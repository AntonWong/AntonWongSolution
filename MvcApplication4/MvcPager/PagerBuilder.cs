...
namespace CYP.Common.MvcPager
{
    ...
    internal class PagerBuilder
    {
        ...
        private const string CopyrightText = "\r\n<!--MvcPager 1.5 for ASP.NET MVC 3.0 © 2014 AntonWang-->\r\n";
        ...
        private string GenerateUrl(int pageIndex)
        {
			...
			#region 重写Controller/Index 去除Index 写法 author:刘斌 crateDate:2013-8-6 10:29:47			
			var uri = urlHelper.RouteUrl(routeValues);

			if (routeValues.ContainsKey("action")
				&& routeValues["action"].ToString().ToLower().Equals("index"))
			{
				int index = uri.IndexOf("?");
				string uriStart = uri.Substring(0, index);
				string uriEnd = uri.Substring(index);

				return string.Format("{0}{1}", uriStart, uriEnd);
			}
        ...
        internal MvcHtmlString RenderPager()
        {
            ...
            {
                sb.Append("<div class='pagination-container'><ul class='pagination'> ");
                foreach (PagerItem item in pagerItems)
                {
        ...
        private MvcHtmlString GeneratePagerElement(PagerItem item)
        {
            ...
            if (item.Disabled) //first,last,next or previous page
                return CreateWrappedPagerElement(item, String.Format("<li><a disabled=\"disabled\">{0}</a></li>", item.Text));
            return CreateWrappedPagerElement(item,
                                             string.IsNullOrEmpty(url)
                                                 ? string.Format("<li class='active'><a>{0}</a></li>", item.Text)
